...
class prepare_data(object):
    ...
    def __init__(self, dataset='JAAD', batch_size=32, cache = True, occlusion = 5, pattern = 0):
        """
        ...
        configs['model_opts']['obs_input_type'] = ['box','speed','center','image', 'box_mean']
        ...
        print("loading dataset: %s..." % self._dataset)
        try: 
        ...
        train_full_data = tabular_transformer(self.train_data, shard=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size())
        train_datasets.append(train_full_data)

        val_dataLoader = DataLoader(tabular_transformer(self.val_data, shard=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size()),batch_size=batch_size, num_workers=1,drop_last=True,shuffle=False)
        self.val_dataLoaders.append(val_dataLoader)

        test_dataLoader = DataLoader(tabular_transformer(self.test_data, shard=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size()),batch_size=batch_size, num_workers=1,drop_last=True,shuffle=False)
        self.test_dataLoaders.append(test_dataLoader)

        for i in range(occlusion):
            train_occl_dataset = tabular_transformer(self.train_data, shard=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size(), occl = i+1, pattern = pattern)
            train_datasets.append(train_occl_dataset)

            val_occl_dataset = tabular_transformer(self.val_data, shard=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size(), occl = i+1, pattern = pattern)
            val_occl_dataLoader = DataLoader(val_occl_dataset, batch_size=batch_size, num_workers=1, drop_last=True,shuffle=True)
            self.val_dataLoaders.append(val_occl_dataLoader)

            test_occl_dataset = tabular_transformer(self.test_data, shard=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size(), occl = i+1, pattern = pattern)
            test_occl_dataLoader = DataLoader(test_occl_dataset, batch_size=batch_size, num_workers=1, drop_last=True,shuffle=True)
            self.test_dataLoaders.append(test_occl_dataLoader)
        print("======Train datasets======")
        train_datasets_all = ConcatDataset(train_datasets)
        self.train_dataLoader = DataLoader(train_datasets_all, batch_size=batch_size, num_workers=1, drop_last=True,shuffle=True)

        for i, train_dataset in enumerate(train_datasets):
            print(f"Lengthof Occl {i}, Lenght of Trainset sample: {len(train_dataset)}")
        print(f"Lenght of Combined Trainset sample: {len(train_datasets_all)}")
        print("======Val & Test datasets======")
        for i, ( valLoader, testLoader) in enumerate(zip(self.val_dataLoaders, self.test_dataLoaders)):
            print(f"Lenghtof Occl: {i}, Length of ValLoader: {len(valLoader)}, Length of TestLoader: {len(testLoader)}")
    ...
    def get_data_sequence(self, data_type, data_raw, opts):
                ...
                for k in d.keys():
                    if k != 'tte':
                        if k != 'box' and k != 'center':
                            ...
                            for i in range(len(d[k])):
                                if k == 'box':
                                    box_mean.append(d[k][i][0])
                                d[k][i] = np.subtract(d[k][i][1:], d[k][i][0]).tolist()
                    d[k] = np.array(d[k])
                d['box_mean'] = np.array(box_mean)
            ...
            for key in d.keys():
                print(f"key: {key}, Shape: {np.shape(d[key])}")
            
            d['crossing'] = np.array(d['crossing'])[:, 0, :]
            d['box_pred'] = np.array(d['box'])[:, 0, :]
            d['center_pred'] = np.array(d['center'])[:, 0, :]
            pos_count = np.count_nonzero(d['crossing'])
            neg_count = len(d['crossing']) - pos_count


            print("="*10 + "Raw data2"+"="*10)
            for key in d.keys():
                print(f"key: {key}, Shape: {np.shape(d[key])}")
            print("="*10 +"="*10)
        ...
        try: 
            ...
            for d_type in model_opts['obs_input_type']:
                if 'local' in d_type or 'context' in d_type:
                    ...
                    print("=== dd: ===")
                    print("type3: {}".format(d_type))
                    features = data[d_type]
                    feat_shape = features.shape[1:]
                    print("shape3: {}".format(feat_shape))
            ...
            if self._generator:
                ...
                _data = (_data, data['crossing'], data['box_pred'], data['center_pred'])
            dd = {'data': _data,
                    'ped_id': data['ped_id'],
                    'image': data['image'],
                    'tte': data['tte'],
...
class tabular_transformer(Dataset):
    ...
    def __init__(self,set_data,transform ="transform_1",shard=0, num_shards=1, occl = 0, pattern=0):
        corssing = set_data['data'][1][shard:][::num_shards]

        corssing_mat = np.zeros((len(corssing),2))
    
        corssing_mat[np.where(corssing==1)[0],1] = 1
        corssing_mat[np.where(corssing==0)[0],0] = 1

        self.corssing_pred = corssing_mat         

        self.bbox = set_data['data'][0][0][shard:][::num_shards]       
        self.speed = set_data['data'][0][1][shard:][::num_shards]      
        self.center = set_data['data'][0][2][shard:][::num_shards]     
        self.bbox_mean = set_data['data'][0][4][shard:][::num_shards]  

        if transform == "transform_1":
            self.transform_1()

        self.X2, self.mask = self.occlu_add_v2(self.X.copy(), occl, pattern)
    ...
    def occlu_add(self, data_seq, occl, type):

        seqNum, obsLen, dim = np.shape(data_seq)
        mask = np.zeros((seqNum, obsLen), dtype=bool)
        if occl:
            for i in range(seqNum):
                random_idx = np.random.choice(obsLen, size=occl, replace=False)
                mask[i, random_idx] = True  
            data_seq[mask] = 0         

        return data_seq, mask

    def occlu_add_v2(self, data_seq, occl, pattern):
        seqNum, obsLen, dim = np.shape(data_seq)
        mask = np.zeros((seqNum, obsLen), dtype=bool)
        ...
        if occl > obsLen:
            raise ValueError("occl must > obsLen")

        for i in range(seqNum):
            available_idx = np.arange(obsLen)  
            selected_idx = []  

            if pattern == 0:  
                while len(selected_idx) < occl and len(available_idx) > 0:
                    candidate = np.random.choice(available_idx)  
                    selected_idx.append(candidate)

                    available_idx = available_idx[
                        (available_idx != candidate) &
                        (available_idx != candidate - 1) &
                        (available_idx != candidate + 1)
                    ]

                mask[i, selected_idx] = True
            elif pattern == 1:  
                start_idx = np.random.randint(0, obsLen - occl + 1)
                mask[i, start_idx : start_idx + occl] = True
        data_seq[mask] = 0
    ...
    def __getitem__(self, idx):
        return self.X[idx].astype('float32'), self.X2[idx].astype('float32'), self.corssing_pred[idx].astype('float32'), self.mask[idx].astype('float32'), self.bbox_mean[idx].astype('float32') #, self.boxc_pred[idx]
